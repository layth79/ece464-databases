...
//Retrieve calendar items from backend
...
function getAssignment(calItems, date, month) {
  ...
  for(let i = 0; i<calItems.length; i++){
    ...
    if(itemDate[1] == date && getMonthFromString(itemDate[2]) == month &&itemDate[3] == y){  //check again for priority 
      ...
      if(name[0].length > 7)
      {
        tmp += '<i id="'+ prior + '" class="fa fa-tag" aria-hidden="true"></i> ' + '<span class="' + prior + '">' + name[0].slice(0, 7) + '</span>' + '.<br>'
      }
      else
      {
        tmp += '<i id="'+ prior + '" class="fa fa-tag" aria-hidden="true"></i> ' + '<span class ="' + prior + '">' + name[0]  + '</span>' + '<br>'
      }
    }
  }
